...
def plot_graph(a=-3, b=-5):
    ## Grid and ecc plot
    drawing = paint_basic_curve(a, b)

    ## Basic plot
    uri0 = render_chart(drawing)

    ## First point
    drawing.text(-2.5, 2.25, "A", weight="bold", color="blue")
    drawing.plot(-1.9, 2, marker="o", markersize=5, markeredgecolor="blue", markerfacecolor="blue")
    plot_text = drawing.annotate(text="From starting point A", xy=(-6, 4), fontsize=12, color="blue")
    uri1 = render_chart(drawing)
    plot_text.remove()

    ## Second point
    drawing.text(-0.3, 2.7, "B", weight="bold", color="blue")
    drawing.plot(-0.25, 2.35, marker="o", markersize=5, markeredgecolor="blue", markerfacecolor="blue")
    plot_text = drawing.annotate(text="pick a point B", xy=(-5.5, 4), fontsize=12, color="blue")
    uri2 = render_chart(drawing)
    plot_text.remove()
    ...
    drawing.plot(x_values, y_values, 'red', linestyle="-")
    drawing.plot(2.1, 2.8, marker="x", markersize=10, markeredgecolor="red", markerfacecolor="red")
    plot_text = drawing.annotate(text="connect A and B with a line", xy=(-6, 4), fontsize=10, color="blue")
    plot_text2 = drawing.annotate(text="the line cuts the curve on a point", xy=(2, 1.5), fontsize=10, color="blue")
    uri3 = render_chart(drawing)
    plot_text.remove()
    plot_text2.remove()

    ## New point
    drawing.text(2.5, 2.8, "P", weight="bold", color="red")
    drawing.plot(2.1, 2.8, marker="o", markersize=5, markeredgecolor="red", markerfacecolor="red")
    plot_text = drawing.annotate(text="we can call this point P", xy=(2, 1.5), fontsize=10, color="blue")
    uri4 = render_chart(drawing)
    plot_text.remove()

    drawing.plot(x_values, y_values, 'black', linestyle="-")
    drawing.plot(2.1, 2.8, marker="x", markersize=5, markeredgecolor="black", markerfacecolor="black")
    point3 = [2.1, 2.8]
    point4 = [2.1, -2.8]
    x_values = [point3[0], point4[0]]
    y_values = [point3[1], point4[1]]
    drawing.plot(x_values, y_values, 'blue', linestyle="--")
    drawing.text(2.5, -2.8, "P' = A + B", weight="bold", color="blue")
    drawing.plot(2.1, -2.8, marker="o", markersize=5, markeredgecolor="blue", markerfacecolor="blue")
    plot_text = drawing.annotate(text="project the P point to the other axis", xy=(2.5, 2.2), fontsize=8, color="blue")
    plot_text2 = drawing.annotate(text="to get the P' point", xy=(2.2, -2), fontsize=8, color="blue")
    uri5 = render_chart(drawing)
    plot_text.remove()
    plot_text2.remove()
...
def same_point_graph(a=-3, b=-5):
    ...
    plot_text2 = plt.annotate(text="The line between these 2 points is the tangent at P", xy=(0.5, 0.5), fontsize=9,
                              color="green")
    uri2 = render_chart(plt)
    plot_text.remove()
    plot_text2.remove()
...
def last_point_graph(a=-3, b=-5):
    ...
    drawing.text(-1, 3, "P", weight="bold", color="green")
    drawing.plot(-0.8, 2.65, marker="o", markersize=10, markeredgecolor="green", markerfacecolor="green")

    drawing.text(-2.5, 2.1, "NP", weight="bold", color="red")
    drawing.plot(-1.98, 1.8, marker="o", markersize=5, markeredgecolor="purple", markerfacecolor="red")
